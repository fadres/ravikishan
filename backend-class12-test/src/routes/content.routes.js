...
// GET /api/subjects/:subjectSlug/chapters/:chapterSlug — structured notes
// with the same per-block degradation rules as the global backend. The
// viewer's tier comes from the shared-JWT payload (req.user.accessLevel).
...
// GET /api/search?q= — search over THIS section's content only. Same
// pipeline + degradation rules as the global backend; results are tagged
// with the section id.
router.get('/search', authenticate, validate(searchSchema, 'query'), async (req, res) => {
  ...
  data.results = data.results.map((r) => ({ ...r, sectionId: SECTION.id }));
...
// GET /api/search/suggestions?q= — lightweight title suggestions.
router.get('/search/suggestions', authenticate, async (req, res) => {
  const q = typeof req.query.q === 'string' ? req.query.q.trim() : '';
  const suggestions = await getSearchSuggestions(q, req.user?.accessLevel ?? 4);
