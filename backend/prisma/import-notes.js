...
//
// Content root (default: import-notes-data/ next to this file):
//   class-11/
//     physics/
//       thermodynamics/
...
//   --allow-create     create missing class/subject/chapter records instead
//                      of failing loudly per file
//   --archive-missing  mark blocks whose source file disappeared as archived
//   --dir <path>       content root (env: NOTES_DIR)
...
import { prisma, SUBJECTS, slugify, humanize, loadJson } from './import-common.js';
import {
  classify,
  validateNoteSchema,
  TAB_TO_BLOCK_TYPE,
...
const DEFAULT_NOTES_DIR = join(HERE, 'import-notes-data');
...
export function parseArgs(argv) {
  ...
  const flags = {
    ...
    dir: process.env.NOTES_DIR || DEFAULT_NOTES_DIR,
  ...
  if (unknown) {
    console.error(`✗ unknown flag: ${unknown}\n  run: npm run import-notes -- --apply [--publish] [--allow-create] [--archive-missing] [--dir <path>] [--threshold <n>]`);
    process.exitCode = 1;
  }
  return flags;
}
...
async function findExistingBlock(chapterId, blockType, title) {
...
async function writePendingVersion(blockId, data) {
 ...
 */
async function upsertBlock(chapter, data, flags, log) {
  const existing = await findExistingBlock(chapter.id, data.blockType, data.title);
  ...
  if (existing.status === 'published' && !flags.publish) {
    ...
    let version = 0;
    if (flags.apply) {
      version = await writePendingVersion(existing.id, data);
  ...
  if (flags.apply) {
    if (flags.publish && !pendingReady) {
      ...
      await writePendingVersion(existing.id, data);
    }
...
async function main() {
  ...
  if (!existsSync(flags.dir)) {
    console.error(`✗ content dir not found: ${flags.dir}\n  place the taxonomy tree there or pass --dir <path>`);
    process.exit(1);
  }
  const runLog = startRunLog(flags.logDir);
  const log = (line) => runLog.write(line);
  log(`run mode=${mode} dir=${flags.dir} publish=${flags.publish} allowCreate=${flags.allowCreate} archiveMissing=${flags.archiveMissing} threshold=${flags.threshold}`);
  console.log(`→ import-notes [${mode}]  dir: ${flags.dir}`);

  const files = discoverNoteFiles(flags.dir);
  const report = { created: 0, updated: 0, pendingVersion: 0, archived: 0, skipped: 0, needsReview: 0, errors: 0 };
  const rows = [];
  const errors = [];
  ...
  for (const entry of plan) {
    ...
    try {
      const resolved = await resolvePlace(parts, flags);
      if (resolved.error) {
        report.errors += 1;
      ...
      const result = await upsertBlock(resolved.chapter, data, flags, log);
  ...
  if (flags.archiveMissing) {
    const stale = await prisma.contentBlock.findMany({
    ...
    for (const b of stale) {
      ...
      report.archived += 1;
      if (flags.apply) {
        await prisma.contentBlock.update({ where: { id: b.id }, data: { status: 'archived' } });
      }
  ...
  // ── Report ──────────────────────────────────────────────────────────────
  printTable(rows);
  console.log(`\n  summary (${mode}): created=${report.created} updated=${report.updated} pending-version=${report.pendingVersion} archived=${report.archived} skipped=${report.skipped} needs-review=${report.needsReview} errors=${report.errors}`);
  if (report.errors) {
    console.log('\n  errors:');
  ...
  log(`done mode=${mode} created=${report.created} updated=${report.updated} pendingVersion=${report.pendingVersion} archived=${report.archived} skipped=${report.skipped} needsReview=${report.needsReview} errors=${report.errors}`);
 ...
 * a path's first three segments. Never writes during dry-run.
 */
async function resolvePlace(parts, flags) {
  const classSlug = slugify(parts.classSlug);
  let klass = await prisma.class.findUnique({ where: { slug: classSlug } });
  if (!klass) {
    if (!flags.allowCreate) return { error: `class "${parts.classSlug}" not found — pass --allow-create` };
    if (flags.apply) {
      klass = await prisma.class.create({ data: { name: humanize(parts.classSlug), slug: classSlug, sortOrder: 0 } });
    } else {
      klass = { id: `__new__${classSlug}`, slug: classSlug };
    }
  }

  const subjectSlug = slugify(parts.subjectSlug);
  let subject = await prisma.subject.findFirst({ where: { classId: klass.id, slug: subjectSlug } });
  if (!subject) {
  ...
  let chapter = await prisma.chapter.findFirst({ where: { subjectId: subject.id, slug: chapterSlug } });
  if (!chapter) {
